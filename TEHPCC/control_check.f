      implicit none
      integer nx,ny,nu,i,j,rc,noS, no_simul,fault_counter,train,test
	integer plot_only_measured
      parameter (nx=26,nu=25,ny=32, rc=3,noS=1000000)
      double precision u(nu),x(nx),dxdt(nx),y(ny),y_ss(ny)
      double precision xls(8),F7,F10,F11
      double precision Cd
      double precision rand1(noS), rand2(noS), rand3(noS), r1_C
      double precision biasvalue,bias_tau,bias_base_gain,biasslope
	  double precision deltat, r3_C, r2_C
	...
	double precision err(3),ctrl_Xm(3)
     ...
     &sensor_bias,setpt_bias,vp_bias,err,ctrl_Xm,r1_C
...
ccc      data gain/1.5,-1.2,-0.05/
	...
	do	fault_counter=1,1		 ! = 0 means no fault at all
	...
	open(unit=2,file='check2.txt',status ='UNKNOWN',access='SEQUENTIAL')
	end if	
...
c	set all the biases to 0
      ...
      read(1,*) (rand1(i),i=1,no_simul)
      close(unit=1)
      

      open(unit=1,file='r2.dat',status='old')
      read(1,*) (rand2(i),i=1,no_simul)
      close(unit=1)

      open(unit=1,file='r3.dat',status='old')
      read(1,*) (rand3(i),i=1,no_simul)
      close(unit=1)
...
c   faults actually being introduced here
	...
	r2_C = rand2(j)
	r3_C = rand3(j)
c      open(unit=1,file='r1_counter.dat',status='replace')
c      write(1,*) r1_Count
c      close(unit=1)
	if(j.eq.1000000) then
...
C- note that velcoity form for control loops will be used
      ...
      double precision errold(3),gain(3),taui(3),setpt(3),interr(3)
	double precision u0(25)
	double precision ctrl_Xm(3),r1_C
      common /CTRL/ setpt,gain,taui,errold,deltat,interr,u0,biasvalue,
      &sensor_bias,setpt_bias,vp_bias,err,ctrl_Xm,r1_C
...
cc		vp_bias(1)=-u0(10)*0.05       !fault_counter=27
	...
	ctrl_Xm(1)=(y(2) + sensor_bias(1))  + r1_C
