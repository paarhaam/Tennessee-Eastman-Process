...
c	*******************************************************************************
	...
	SUBROUTINE TEsimf(nx,nu,ny,x,u,y,dxdt,Cd,xls,F7)
...
c		Rate Laws.  All rates in [kmol/h].
      ...
      RR(1)=Cd*VVR*DEXP(44.06-42600.0/1.987/TKR)*R1F*(PPR(1)**1.08)
     +		*(PPR(3)**0.311)*(PPR(4)**0.874)
      RR(2)=Cd*VVR*DEXP(10.27-19500.0/1.987/TKR)*R2F*(PPR(1)**1.15)
     +		*(PPR(3)**0.370)*(PPR(5)**1.00)
      RR(3)=Cd*VVR*DEXP(59.50-59500.0/1.987/TKR)*PPR(1)
     +		*(0.77*PPR(4)+PPR(5))
...
c		Pressure drop vs. flow to get flows of streams 6 & 7.
	...
	if (ptr .gt. ptv) then
		F6=-F6
		write(*,*) 'problem here with F6'
	end if
	F7=(5722.0/wtmolr)*sqrt(abs(PTR-PTS))*RSflow
	if (pts .gt. ptr) then
		F7=-F7
	        write(*,*) 'Problem here with F7'
	end if
...
cc	write(*,*) 'xls(5)=',xls(5)
cc	write(*,*) 'F7=',F7,'  press ^c. this is done just to get F7'
cc	read(*,*) F7 ! used to calculate the base value of F7
	return
	end
